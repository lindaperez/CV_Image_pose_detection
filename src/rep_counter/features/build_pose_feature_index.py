...
- original RepCount split files under ``datasets/metadata/llsp`` where the split
...
def default_annotation_csvs() -> list[Path]:
    cleaned = sorted(METADATA_DIR.glob("*_cleaned.csv"))
    if cleaned:
        return cleaned
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "otherwise falls back to raw split files under datasets/metadata/llsp."
        ),
    )
    parser.add_argument(
        "--exercise",
