...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=PROJECT_DIR,
        help="Repo root to serve. Defaults to the current ML_System project root.",
    )
    return parser
        ...
        if normalized_path == "/api/health":
            self._send_json(
                HTTPStatus.OK,
                {
                    ...
                    "api_base": "/api",
                    "default_manifest_path": "outputs/training_outputs/hard_case_review_manifest.csv",
                },
            )
            return
...
def run_server(host: str, port: int, project_dir: Path) -> None:
    ...
    with ThreadingHTTPServer((host, port), handler) as httpd:
        ...
        print("Review app: /reports/dashboards/hard_case_review_app.html")
