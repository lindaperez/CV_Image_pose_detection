(function () {
  ...
  const DEFAULT_ANNOTATION_BASE_PATH = `${REPO_ROOT_PATH}datasets/metadata/llsp/`;
  ...
  function buildManifestViewerHref(relativePath) {
    ...
    return `${REPO_ROOT_PATH}reports/dashboards/hard_case_review_manifest_viewer.html?${params.toString()}`;
  }
    ...
    try {
      ...
      if (payload.annotationBasePath) {
        DOM.annotationBasePath.value =
          payload.annotationBasePath === "/datasets/metadata/llsp/"
            ? DEFAULT_ANNOTATION_BASE_PATH
  ...
  if (!DOM.annotationBasePath.value || DOM.annotationBasePath.value === "/datasets/metadata/llsp/") {
